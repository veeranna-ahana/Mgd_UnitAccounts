...
export default function Open() {
  ...
  const OpenReceipts = async () => {
    try {
      const response = await axios.get(baseURL + '/Payment_Receipts/getopenreceipts'); // Replace this URL with your API endpoint
          ...
          <tbody className="tablebody">
            {sortedData() ? sortedData().map((rv, key) => (
              <tr
                ...
                className={key === selectRow?.index ? 'selcted-row-clr' : ''} key={rv.RecdPVID}
